...
These functions should be passed as `event_consumer` arguments to `Engine()`.
...
```python
    Engine(..., event_consumer=yapapi.log.log_event)
```
For even more detailed, machine-readable output use `log_event_repr`:
```python
    Engine(..., event_consumer=yapapi.log.log_event_repr)
```
For summarized, human-readable output use `log_summary()`:
```python
    Engine(..., event_consumer=yapapi.log.log_summary())
```
...
class SummaryLogger:
    ...
    ```python
        ...
        engine = Engine(..., event_consumer=summary_logger)
    ```
    """
